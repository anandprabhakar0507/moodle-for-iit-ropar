<?php 
	...
	if($_POST['function_num']==1){
		...
		$query3= "SELECT staff_id,password FROM staff_dean_academics WHERE staff_id='$username'";
		...
		require 'db_connect.php'; 
		$username=$_POST['username'];
		$query3= "SELECT email FROM staff_dean_academics WHERE staff_id='$username'";
		$query_handle=mysql_query($query3);	
		if(mysql_num_rows($query_handle)==1){
			$reply=mysql_fetch_assoc($query_handle);
			session_start();
